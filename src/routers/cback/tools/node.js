 ...
 */
async function findFirstNodeByName(core, root, searchName, containerPathNorm, maxDepth) {
 ...
 */
async function resolveNodePathOrName(core, root, pathOrName, containerPath) {
    const s = pathOrName == null ? "" : String(pathOrName).trim();
    if (s === "" || s === PATH_SEP)
        return core.getPath(root);
    const containerNorm = normalizePath(containerPath !== null && containerPath !== void 0 ? containerPath : "");
    if (s.charAt(0) === PATH_SEP) {
        ...
        if (namePart)
            return findFirstNodeByName(core, root, namePart, containerNorm, RESOLVE_NAME_MAX_DEPTH);
        return null;
    }
...
exports.createNode = {
    definition: {
        name: "createNode",
        description: "Create a new node in the WebGME model (server-side). Use this only for creating *instance* nodes in the model, not for defining new concepts. " +
        ...
        parameters: {
            ...
            properties: {
                baseType: {
                    type: "string",
                    description: "Optional. Omit unless user specifies a type. Must be FCO or an existing node path (e.g. from findNodesByName or getMetaInfo); do not invent type names.",
                },
                container: {
                    type: "string",
...
exports.moveNode = {
    definition: {
        name: "moveNode",
        description: "Move a node to a new parent (container). The move is performed on the server and committed. " +
        ...
        parameters: {
            ...
            properties: {
                ...
                newContainer: {
                    type: "string",
                    description: "The new parent node: path (e.g. /1) or name (e.g. META). Resolved on the backend. If omitted, activeNodeId is used.",
                },
            },
            required: ["nodeId"],
        },
    ...
    handler: async (args, ctx) => {
        ...
        const newContainerRaw = (_a = args.newContainer) !== null && _a !== void 0 ? _a : getActiveNodeId(ctx);
        if (!newContainerRaw) {
            return {
                data: {
        ...
        try {
            ...
            const nodeName = (_b = core.getAttribute(node, "name")) !== null && _b !== void 0 ? _b : nodeIdRaw;
...
exports.getProperty = {
    definition: {
        name: "getProperty",
        description: "Get property/properties of a node. With no name: lists all properties (attributes and registry) with current values. With name: returns that property's value. " +
    ...
    handler: async (args, ctx) => {
        ...
        try {
            ...
            for (const n of attrNames)
                attributeValues[n] = formatPropertyValue(core.getAttribute(node, n));
            for (const n of regNames)
                registryValues[n] = formatPropertyValue(core.getRegistry(node, n));
            const valueFormats = {};
            for (const key of [...attrNames, ...regNames]) {
                const v = key in attributeValues ? attributeValues[key] : registryValues[key];
                if ((v === undefined || v === "") && VALUE_FORMAT_HINTS[key])
...
exports.setProperty = {
    definition: {
        name: "setProperty",
        description: "Set a property on a node. The backend determines whether it is an attribute or registry entry (attributes have priority). " +
...
exports.setAttribute = {
    definition: {
        name: "setAttribute",
        description: "Set an attribute value on a node. Use ONLY when getPropertyNames shows the property in 'attributes'. " +
...
exports.getAttribute = {
    ...
    handler: async (args, ctx) => {
        ...
        if (ctx.coreSession) {
            ...
            if (resolved)
                nodeIdForCommand = toDisplayPath(resolved);
        }
        return {
            data: {
...
exports.setRegistry = {
    definition: {
        name: "setRegistry",
        description: "Set a registry entry on a node. Use ONLY when getPropertyNames shows the property in 'registry'. " +
...
exports.bulkSet = {
    definition: {
        name: "bulkSet",
        description: "Set attributes and registry entries on multiple nodes in one go, then save and commit once. " +
        ...
        parameters: {
            ...
            properties: {
                nodes: {
                    type: "array",
                    description: "Array of { path: string, attributes?: Record<string, string>, registry?: Record<string, value> }. path is the node path (e.g. '/1' or '/1/2').",
                    items: {
                        type: "object",
                        properties: {
    ...
    handler: async (args, ctx) => {
        if (!ctx.coreSession) {
            return {
                data: {
                    error: "Project context is required for bulkSet. The client must send projectId (and optionally branchName) in the request context.",
                },
            };
        }
        ...
        try {
            for (const item of nodesArg) {
                const pathRaw = (item === null || item === void 0 ? void 0 : item.path) != null ? String(item.path).trim() : "";
                ...
                if (item.attributes && typeof item.attributes === "object") {
                    for (const [name, value] of Object.entries(item.attributes)) {
                        ...
                        if (res) {
                            errors.push(toDisplayPath(nodePath) + " setAttribute(" + name + "): " + (res.message || String(res)));
                        }
                    }
                }
                if (item.registry && typeof item.registry === "object") {
                    for (const [name, value] of Object.entries(item.registry)) {
                        ...
                        const res = core.setRegistry(node, name, valueToSet);
                        if (res) {
                            errors.push(toDisplayPath(nodePath) + " setRegistry(" + name + "): " + (res.message || String(res)));
                        }
                    }
                }
...
exports.getDiagramLayout = {
    ...
    handler: async (_args, ctx) => {
        var _a;
        const layout = (_a = ctx.context) === null || _a === void 0 ? void 0 : _a.diagramLayout;
