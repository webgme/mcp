...
function ensureCoreSession(ctx) {
    if (!ctx.coreSession) {
        throw new Error("Meta tools require an open project with coreSession. Ensure the client sends projectId (and branch) in context.");
    }
}
/**
 ...
 */
function getActiveSheetSetId(ctx) {
    var _a, _b, _c;
    if (!ctx.coreSession)
        return null;
    ...
    const tabIndex = typeof ((_a = ctx.context) === null || _a === void 0 ? void 0 : _a.activeTabId) === "number" ? ctx.context.activeTabId : 0;
    const sheet = sheets[tabIndex];
    return sheet && sheet.SetID ? sheet.SetID : (_c = (_b = sheets[0]) === null || _b === void 0 ? void 0 : _b.SetID) !== null && _c !== void 0 ? _c : null;
}
/**
 ...
 */
async function resolveMetaPathOrName(core, root, pathOrName) {
    const s = String(pathOrName !== null && pathOrName !== void 0 ? pathOrName : "").trim();
    if (!s)
        return null;
    if (s === "/")
        return core.getPath(root);
...
exports.createMetaSheet = {
    definition: {
        name: "createMetaSheet",
        description: "Create a new meta aspect sheet for the current project. " +
        ...
        parameters: {
            ...
            properties: {
                title: {
                    type: "string",
                    description: "Optional title for the new sheet. Defaults to 'New sheet' if omitted.",
                },
            },
            required: [],
        },
...
exports.switchToMetaSheet = {
    definition: {
        name: "switchToMetaSheet",
        description: "Select a meta aspect sheet for subsequent meta operations (server-side selection only; does not change the browser UI). " +
        ...
        parameters: {
            ...
            properties: {
                setId: {
                    type: "string",
                    description: "SetID of the sheet (e.g. 'MetaAspectSet_...'). If provided, this takes precedence over title.",
                },
                title: {
                    type: "string",
                    description: "Optional sheet title to resolve when setId is not given.",
                },
            },
            required: [],
        },
...
exports.deleteMetaSheet = {
    definition: {
        name: "deleteMetaSheet",
        description: "Delete a meta aspect sheet. Runs on the server: removes the sheet set on the ROOT, updates the MetaSheets registry, " +
        ...
        parameters: {
            ...
            properties: {
                setId: {
                    type: "string",
                    description: "SetID of the meta sheet to delete (e.g. 'MetaAspectSet_...').",
                },
            },
            required: ["setId"],
        },
    ...
    handler: async (args, ctx) => {
        ...
        try {
            const rawSheets = core.getRegistry(root, META_SHEETS_REGISTRY) || [];
            let sheets = Array.isArray(rawSheets) ? rawSheets.slice() : [];
            const sheetIndex = sheets.findIndex((s) => s.SetID === setId);
            if (sheetIndex === -1) {
                return {
                    data: {
                        error: "Meta sheet not found: " +
                            setId +
            ...
            for (const path of itemsOfAspect) {
                ...
                if (count === 1) {
                    ...
                    const isLib = typeof core.isLibraryElement === "function" &&
...
exports.isMetaNodeTool = {
    definition: {
        name: "isMetaNode",
        description: "Check if a node is a META node (i.e. a member of the global MetaAspectSet). " +
        ...
        parameters: {
            ...
            properties: {
                nodePath: {
                    type: "string",
                    description: "Absolute path of the node to check ('' or '/' for the project root).",
                },
            },
            required: ["nodePath"],
        },
    ...
    handler: async (args, ctx) => {
        ensureCoreSession(ctx);
        const { core, root } = ctx.coreSession;
        const rawPath = args.nodePath;
        const path = String(rawPath !== null && rawPath !== void 0 ? rawPath : "").trim();
        try {
            ...
            const isMeta = typeof core.isMetaNode === "function"
...
exports.createMetaNode = {
    definition: {
        ...
        parameters: {
            ...
            properties: {
                ...
                basePath: {
                    type: "string",
                    description: "Optional. Base concept name (e.g. FCO) or path from getMetaInfo. Omit to use FCO.",
                },
                contains: {
                    type: "array",
                    description: "Optional. Types this concept can contain. Each item: { targetPath: string, min?: number, max?: number }. targetPath = concept name or path (e.g. FCO). Omit min/max for any amount.",
                    items: {
                        type: "object",
                        properties: {
                ...
                sets: {
                    type: "array",
                    description: "Optional. Sets on this concept (multi-reference). Each item: { setName: string, targetPath: string, min?: number, max?: number }. Omit min/max for no limit.",
                    items: {
                        type: "object",
                        properties: {
    ...
    handler: async (args, ctx) => {
        ...
        const name = String(nameRaw !== null && nameRaw !== void 0 ? nameRaw : "").trim();
        if (!name) {
        ...
        try {
            ...
            let baseNode = null;
            const basePathRaw = args.basePath;
            const basePath = basePathRaw != null ? String(basePathRaw).trim() : "";
            if (basePath) {
                ...
                const candidate = resolvedPath === core.getPath(root) ? root : await core.loadByPath(root, resolvedPath);
                if (!candidate) {
                    return {
                        data: {
                ...
                if (!baseNode) {
                    return {
                        data: {
                            error: "Could not resolve FCO for default base when creating meta node.",
                        },
                    };
                }
            }
            ...
            if (Array.isArray(containsArg) && containsArg.length > 0) {
                for (const item of containsArg) {
                    ...
                    if (res) {
                        ctx.logger.warn("createMetaNode setChildMeta: " + res.message);
                        continue;
                    }
...
exports.setMetaAttribute = {
    definition: {
        ...
        parameters: {
            ...
            properties: {
                conceptPath: {
                    type: "string",
                    description: "Absolute path of the concept (meta-node) whose attribute meta to set.",
                },
                attributeName: {
                    type: "string",
                ...
                rule: {
                    type: "object",
                    description: "Attribute meta rule object (type, enum, min, max, default, etc.), passed directly to core.setAttributeMeta.",
                },
            },
    ...
    handler: async (args, ctx) => {
        ensureCoreSession(ctx);
        const { core, root } = ctx.coreSession;
        const conceptPathRaw = args.conceptPath;
        const conceptPath = String(conceptPathRaw !== null && conceptPathRaw !== void 0 ? conceptPathRaw : "").trim();
        const attributeNameRaw = args.attributeName;
        const attributeName = String(attributeNameRaw !== null && attributeNameRaw !== void 0 ? attributeNameRaw : "").trim();
        ...
        try {
            const node = conceptPath === "/" ? root : await core.loadByPath(root, conceptPath);
            if (!node) {
                return {
            ...
            if (res) {
                ...
                return {
                    data: {
                        error: "setAttributeMeta reported an error: " +
                            res.message,
                    },
                };
            }
...
exports.setMetaContainment = {
    definition: {
        ...
        description: "Define a META-level containment: a concept can contain instances of another (e.g. 'A can contain B'). Uses core.setChildMeta. " +
        ...
        parameters: {
            ...
            properties: {
                sourcePath: {
                    type: "string",
                    description: "Container concept path (e.g. /SM) or concept name. Use leading '/' for path; otherwise treated as name.",
                },
                targetPath: {
                    type: "string",
                    description: "Contained concept path (e.g. /Item) or concept name. Use leading '/' for path; otherwise treated as name.",
                },
                min: {
                    type: "number",
                    description: "Optional minimum cardinality. If the user says 'any' or does not specify cardinality, omit this (do not send).",
                },
                max: {
                    type: "number",
                    description: "Optional maximum cardinality. If the user says 'any' or does not specify cardinality, omit this (do not send).",
                },
            },
    ...
    handler: async (args, ctx) => {
        ensureCoreSession(ctx);
        const { core, root } = ctx.coreSession;
        const sourcePathRaw = args.sourcePath;
        const targetPathRaw = args.targetPath;
        const sourcePathOrName = String(sourcePathRaw !== null && sourcePathRaw !== void 0 ? sourcePathRaw : "").trim();
        const targetPathOrName = String(targetPathRaw !== null && targetPathRaw !== void 0 ? targetPathRaw : "").trim();
        ...
        try {
            ...
            const isSourceMeta = typeof core.isMetaNode === "function"
                ? core.isMetaNode(sourceNode)
                : false;
            const isTargetMeta = typeof core.isMetaNode === "function"
                ? core.isMetaNode(targetNode)
                : false;
            if (!isSourceMeta || !isTargetMeta) {
                return {
                    data: {
                        error: "Both source and target must be META nodes. " +
            ...
            if (res) {
                return {
                    data: {
                        error: "setChildMeta reported an error: " +
                            res.message,
                    },
                };
            }
...
exports.setMetaPointer = {
    definition: {
        name: "setMetaPointer",
        description: "Define or change a META-level pointer on an **existing** concept (use conceptPath from getMetaInfo)—do not create a new concept with createMetaNode when the user only wants new pointers. " +
            "Instances can reference at most one target per pointer. Adds a valid target type for the pointer. Uses min=1, max=1 so it is displayed as a pointer (not a set). " +
...
exports.setMetaSet = {
    definition: {
        name: "setMetaSet",
        description: "Define a META-level set on a concept: instances can reference multiple targets. " +
...
exports.setMetaMixin = {
    definition: {
        name: "setMetaMixin",
        description: "Add a META mixin to a concept: the concept inherits from the mixin type (attributes, containment, pointers, etc.). " +
    ...
    handler: async (args, ctx) => {
        ...
        try {
            ...
            if (!conceptNode)
                return { data: { error: "Concept not found at " + conceptPath } };
            if (!mixinNode)
                return { data: { error: "Mixin concept not found at " + mixinPath } };
...
exports.getMetaInfo = {
    definition: {
        name: "getMetaInfo",
        description: "Collect meta information for the current project. Runs on the server. " +
            ...
            "Call before createMetaNode or when editing the metamodel so you reuse concepts[].path and avoid duplicate concept names.",
        parameters: {
            type: "object",
            properties: {},
            required: [],
    ...
    handler: async (_args, ctx) => {
        ...
        try {
            ...
            for (let i = 0; i < conceptPaths.length; i++) {
                ...
                if (!node)
                    continue;
                const name = (_a = core.getAttribute(node, "name")) !== null && _a !== void 0 ? _a : "";
                let meta = null;
                try {
                    meta = core.getJsonMeta(node);
                }
                catch (e) {
                    ctx.logger.warn("getMetaInfo: getJsonMeta failed for " +
                        path +
                        ": " +
                        (e && e.message));
            ...
            const rawSheets = core.getRegistry(root, META_SHEETS_REGISTRY) || [];
            const sheetsArray = Array.isArray(rawSheets)
                ? rawSheets.slice()
                : [];
            const sheets = sheetsArray.map((s) => {
                const setId = s.SetID;
                const order = typeof s.order === "number" ? s.order : 0;
                const title = s.title || setId;
                const memberPaths = (setId && core.getMemberPaths(root, setId)) || [];
...
exports.setConceptPosition = {
    definition: {
        ...
        description: "Set the diagram position of a single concept on the active meta sheet. " +
 ...
 */
exports.setConceptLayout = {
    definition: {
        name: "setConceptLayout",
        description: "Set the diagram positions of multiple concepts on the active meta sheet in one go, then commit once. " +
    ...
    handler: async (args, ctx) => {
        ...
        try {
            ...
            for (const item of nodesArg) {
                const pathOrName = (item === null || item === void 0 ? void 0 : item.path) != null ? String(item.path).trim() : "";
