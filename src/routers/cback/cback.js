    ...
    router.post("/chat", express_1.default.json(), async function (req, res) {
        ...
        if (context && typeof context === "object") {
            ...
            if (context.activeVisualizerId)
                ctxParts.push("activeVisualizerId=" + context.activeVisualizerId);
        ...
        try {
            ...
            while (rounds < maxToolRounds) {
                ...
                const result = useAnthropic
                    ? await (0, anthropic_1.chatCompletion)(history, toolDefs, { apiKey: anthropicApiKey, model: anthropicModel })
                    : useGroq
                        ? await (0, openai_1.chatCompletion)(history, toolDefs, { apiKey: groqApiKey, model: groqModel, baseUrl: openai_1.GROQ_BASE_URL })
                        : useOpenAI
                            ? await (0, openai_1.chatCompletion)(history, toolDefs, { apiKey: openaiApiKey, model: openaiModel, baseUrl: openai_1.OPENAI_BASE_URL })
                            : await (0, ollama_1.chatCompletion)(history, toolDefs, ollamaConfig);
                const msg = result.message;
                if (result.usage) {
                    logger.info("cback tokens round " + rounds + ": prompt=" + result.usage.prompt_tokens +
